...
public class TARGZ {
  ...
  private static void addFileToTar(Path file, Path targetFile, TarArchiveOutputStream tOut)
      throws IOException {
    TarArchiveEntry tarEntry = new TarArchiveEntry(file.toFile(), targetFile.toString());

    // setting default values for tar meta data to prevent misleading changes between final .tgz
    // files when regenerating snapshots
