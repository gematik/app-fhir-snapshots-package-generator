...
public class DependencyGenerator {
  ...
  public List<String> generateListOfDependenciesFor(String filename, String packageFolderPath)
      throws IOException {
    ...
    if (!dependencyList.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          String.format("Could not generate dependencies for %s", filename));
    }
  }

  private List<String> getDependencyList(String filename, String packageFolderPath)
      throws IOException {
  ...
  private void getDependenciesFromPackageJson(
      String packageFolderPath, List<String> dependencyList, String filename) throws IOException {
    List<PackageReference> additionalRefs =
        getDependenciesFor(packageFolderPath + File.separator + filename);
  ...
  private String getPackageFilename(PackageReference pr, String packageFolderPath) {
    ...
    if (pr.getPackageVersion().toLowerCase().endsWith("x")) {
      ...
      if (StringUtils.isEmpty(packageFilename)) {
        throw new IllegalArgumentException(
            String.format(
                "Wildcard version '%s' has been found for package name '%s'. But no matching packages were found in '%s'",
                pr.getPackageVersion(), pr.getPackageName(), packageFolderPath));
      }
    } else {
      packageFilename =
          String.format("%s-%s.tgz", pr.getPackageName(), pr.getPackageVersion()).toLowerCase();
    }
    ...
    try (TarArchiveInputStream tarInputStream =
        new TarArchiveInputStream(
            new GzipCompressorInputStream(
                new BufferedInputStream(new FileInputStream(tgzFilePath))))) {
