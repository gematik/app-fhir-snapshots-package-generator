...
/*-
 * #%L
 * fhir-snapshots-package-generator-cli
 * %%
...
public class SnapshotGeneratorCli implements Runnable {
  ...
  @Option(
      ...
      split = ",",
      description =
          "Comma-separated list of package names to generate snapshots for. If omitted, all source FHIR packages are processed.")
  private final List<String> packageNames = Collections.emptyList();

  @Option(
      names = "--tempDir",
      description =
          "The temporary directory for decompressing the FHIR packages. Default is system temp directory.")
    ...
    try {
      if (packageNames.isEmpty()) {
        ...
        snapshotGenerator.generateSnapshots(
            packageFolderPath, outputFolderPath, packageNames, tempDir);
      }
    } catch (IOException e) {
